...
# jna: add duty-cycle - this is when there is a schedule on the nest itself for the fan.
# I guess you should be doing this in Indigo, but hey, why crash if this is set!
NEST_FAN_MAP={'auto on':"auto",
              'on': "on",
              'auto': "auto",
              'always on': "on",
              '1': "on",
...
# One day, Nest decided to only support TLSv1. Urllib2, requests, and
# other python libraries break for TLSv1, so, we have to write our
# own. They're doing this on the status and command URLs only, not the
# login URLs, so we can continue to use urllib for the login/auth

def sendandlog(connection,data):
  connection.send(data)
  # jna: uncomment the next line to log everything to the indigo log, for debugging.
  # indigo.server.log(u"%s" % data)

def http_tlsv1(url, headers=None, data=None):
  # Very dirty TLSv1 HTTP client implementation
  # Prefer TLS
  ...
  connection.connect((urlparts.hostname,urlparts.port))
  ...
  # adjust for proper method here
  method = "GET"
  headers.pop('content-length', 0)
  ...
  sendandlog(connection, method + " " + urlparts.path + " HTTP/1.1\r\nHost: " + urlparts.hostname)

  for k in headers.keys():
    sendandlog(connection, "%s: %s\r\n" % (k,headers[k]))

  sendandlog(connection, "Connection: Close\r\n")
  sendandlog(connection, "\r\n")

  if data != None:
    sendandlog(connection, data) 
...
class NestThermostat:
  ...
  def _refresh_status(self):
    ...
    if (not self._cached or (time.time()-self._last_update>NEST_CACHE_REFRESH_TIMEOUT)):
      ...
      mydata = http_tlsv1(self._status_url,headers=self._header)
      self._status_data=json.loads(mydata)
      ...
      # Use this to set the serial and structure (location) instance
      # variables and construct the URLs.  I'd rather do this earlier,
      # but letting the user refer to the Nest (and its location) by
      # name is worth it.
  ...
  def _send_command(self,command,url):
    ...
    discard_me=""
    try:
      discard_me=http_tlsv1(url, data=command, headers=self._header)
    except:
      # Do nothing
      pass
    return discard_me
